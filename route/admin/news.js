...
module.exports = function () {
            ...
            case 'del':
                db.query(`SELECT * FROM news_table WHERE ID='${req.query.id}'`, (err, data) => {
                    if (err) {
                        ...
                        if (data.length == 0) {
                            res.status(404).send('no this news evaluation').end();
                        } else {
            ...
            case 'mod':
                db.query(`SELECT * FROM news_table WHERE ID = '${req.query.id}'`, (err, data) => {
                    if (err) {
                        ...
                        db.query(`SELECT * FROM news_table`, (err, newsdata) => {
                            if (err) {
                                console.error(err);
                                req.status(500).send('database erro mod2').end();
                            } else {
                                res.render('admin/news.ejs', { newsdata, mod_data: data[0] });
                            }
                        });
                    }
                });
            ...
            default:
                db.query(`SELECT * FROM news_table`, (err, newsdata) => {
                    if (err) {
                        ...
                        res.render('admin/news.ejs', { newsdata });
                    }
                });
        }
    });
    ...
    router.post('/', function (req, res) {
        ...
        if (newFileName) {
                ...
                if (err) {
                    ...
                    if (req.body.mod_id) {  //修改
                            ...
                            if (err) {
                                ...
                                fs.unlink('static/upload/' + data[0].url, (err) => {
                                    if (err) {
                                        ...
                                        db.query(`UPDATE news_table SET title='${title}', second_title='${second_title}', url='${newFileName}', content='${content}', author='${author}',post_time='${post_time}' 
                      WHERE ID='${req.body.mod_id}'`, (err) => {
                        ...
                        db.query(`INSERT INTO news_table (title, second_title, url, content, author, post_time,type)
            ...
            if (req.body.mod_id) {  //修改
                //直接改，除了没有图片外的东西
                db.query(`UPDATE news_table SET title='${title}',second_title='${second_title}',content='${content}',author='${author}',post_time='${post_time}',type='${type}' WHERE ID=${req.body.mod_id}`, (err) => {
                    if (err) {
                        console.error(err);
